"""
=============================================================================
Module:        Test — Analyser Pipeline
Location:      tests/test_analyser.py
Description:   Unit tests for core/analyser.py — the request entry
               point that takes user input (product photos, meal
               descriptions, menu scans, chat messages, onboarding
               turns) and dispatches each to the appropriate LLM
               prompt + router. All LLM router calls are mocked.

Architecture Note:
Five public entry points exercised:
  - analyse_product   — barcoded item with allergen profile
  - analyse_meal      — meal photo, FODMAP / general dietary checks
  - analyse_menu      — restaurant menu OCR scan
  - chat_assistant    — open-ended dietary chat
  - onboarding_assistant — onboarding-flow conversational turns

Mock router responses use the verbatim flat-text schema the LLMs
emit (Key: Value lines, --- terminator). Some mock strings exceed
the 140-char line limit because they are wire-format payloads;
wrapping them mid-string would change the value being asserted.
Per-line line-too-long disables mark this intent.
=============================================================================
"""
